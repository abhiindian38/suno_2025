                    ...
                    songs.map((song, index) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full group relative flex items-center gap-3 sm:gap-6 p-3 sm:p-4 rounded-xl sm:rounded-2xl transition-all duration-500 overflow-hidden ${isActive
                                    ? 'bg-white/5 shadow-2xl ring-1 ring-white/10'
                                    : 'hover:bg-white/[0.03]'
                                    }`}
                            >
                                <div className="relative shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl overflow-hidden border border-white/5">
                                    ...
                                    {isActive && (
                                        <div className="absolute inset-0 bg-purple-500/60 flex items-center justify-center backdrop-blur-sm">
                                            <div className="flex gap-1 items-end h-3 sm:h-4">
                                                <div className="w-0.5 sm:w-1 bg-white rounded-full animate-[music-bar_0.6s_infinite]" />
                                                <div className="w-0.5 sm:w-1 bg-white rounded-full animate-[music-bar_0.9s_infinite_0.1s]" />
                                                <div className="w-0.5 sm:w-1 bg-white rounded-full animate-[music-bar_0.7s_infinite_0.2s]" />
                                            </div>
                                        </div>
                                    )}
                                    <div className={`absolute inset-0 bg-black/60 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex items-center justify-center ${isActive ? 'hidden' : ''}`}>
                                        <Play className="w-3 h-3 sm:w-4 sm:h-4 text-white fill-current" />
                                    </div>
                                </div>

                                <div className="flex-1 min-w-0 text-left">
                                    <h4 className={`text-xs sm:text-sm font-bold truncate transition-colors ${isActive ? 'text-purple-400' : 'text-white/80 group-hover:text-white'}`}>
                                        {song.name}
                                    </h4>
                                    <p className="text-[9px] sm:text-[10px] font-medium text-white/30 truncate uppercase tracking-wider">
                                        {song.artists.join(', ')}
                                    </p>
                                </div>
                                ...
                                <div className="shrink-0 flex items-center gap-2 sm:gap-4 text-right">
                                    {isActive && (
                                        <div className="hidden xs:flex items-center gap-2 text-purple-500/50">
                                            <Volume2 className="w-3 h-3 animate-pulse" />
                                            <span className="text-[9px] font-black uppercase tracking-[0.2em]">Playing</span>
                                        </div>
                                    )}
                                    <span className={`text-[9px] sm:text-[10px] font-bold tabular-nums tracking-tighter ${isActive ? 'text-purple-500/40' : 'text-white/10 group-hover:text-white/30'}`}>
